...
getConfiguration = function() {
  /*
    ...
    #
    */  var config;
  if (fs.existsSync('.dracularc.js')) {
    config = require('.dracularc.js');
    return config;
  }
  if (fs.existsSync(root + 'configuration.js')) {
    /*
...
module.exports.Dracula = function(args) {
  ...
  exec("" + phantomjs + " " + phantomlib + " " + screenshots + " " + screenshot_x + " " + screenshot_y + " " + screenshot_dir + " " + args[2], function(error, stdout, stderr) {
    if (stdout) {
      console.log(stdout);
      say("Moving screenshots", green);
      if (screenshots) {
        exec("mv " + test_dir + "/*.png " + screenshot_dir, function(error, stdout, stderr) {
          if (stderr) {
            say(stderr, red);
            return say("Moving the screenshot faild ?!", red);
          }
        });
      }
    }
    if (stderr) {
      return say(stderr, red);
    }
  });
